...
python evaluation_script.py -t tasks.json -o results.json --wv-network-logs /path/to/webvoyager/networklogs --wv-interact-messages /home/aianta/shock_and_awe/WebVoyager/results/20251014_17_54_50

To Evaluate OdoBot results
python evaluation_script.py -t tasks.json -o results.json --odobot-execution-events /path/to/odobot/networklogs 

To Evaluate Single OdoBot result
python evaluation_script.py -t tasks.json -o result.json --single-odobot-execution-events path/to/task/execution_events.json --single-odobot-task-query-construction path/to/task_query_construction.json

To Evalute Agent E results
python evaluation_script.py -t tasks.json -o agent_e_results.json --agent-e-network-logs /path/to/agente/network/logs 
'''
...
# Support for evaluating agent e task execution network logs
parser.add_argument("--agent-e-network-logs", 
                    dest="agent_e_network_logs",
                    help="Path to the directory containing agent e network logs collected while executing tasks.",
...
if args.odobot_execution_events:
    ...
    with os.scandir(args.odobot_execution_events) as _dir:
        for entry in _dir:
            if entry.name.endswith('.json') and 'task-query' not in entry.name and 'history' not in entry.name and 'path-selection-result' not in entry.name: #If it is a json file, try and parse it as a OdoBotExecutionEventLog
            ...
            if entry.name.endswith('.json') and 'task-query' in entry.name and 'history' not in entry.name and 'path-selection-result' not in entry.name:
                with open(entry.path, 'r') as tqc_file:
                    task_query_construction_result = json.load(tqc_file)
                    task_instance_id = next(x for x in Task.ALL_TASK_INSTANCES if x in entry.path)
                    evaluator.register_odobot_target(task_instance_id, task_query_construction_result['targets'][0] )

if args.agent_e_network_logs:
    print(f"Looking for Agent E Network Logs in: {args.agent_e_network_logs}")
    with os.scandir(args.agent_e_network_logs) as _dir:
        for entry in _dir:
            if entry.name.endswith('network_logs.json'):
                network_log = AgentENetworkEventLog.to_execution_event_log(entry.path)
