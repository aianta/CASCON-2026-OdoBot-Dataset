...
class NetworkEvent:
    ...
    def from_agent_e_event(raw_event):
        method = raw_event["method"]

        parse_result = urlparse(raw_event["url"])
        ...
        if "postData" in raw_event and raw_event["postData"] is not None:
            try:
                ...
                print(f"PostData was:\n{raw_event['postData']}")

                request_data = parse_qs(raw_event["postData"])
...
class AgentENetworkEventLog:

    @staticmethod
    def to_execution_event_log(path):
        for instance_id in Task.ALL_TASK_INSTANCES:
            if instance_id in path:
                return AgentENetworkEventLog(open(path, 'r', encoding="utf-8", errors="ignore"), instance_id)
    ...
    def __init__(self, file, instance_id):
        ...
        self.network_events = [NetworkEvent.from_agent_e_event(x) for x in self.events]
...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for instance_id in self.network_events:
            ...
            num_network_requests += result['num_network_requests']
    ...
    def evaluate_against_answer(self, instance, instance_reference_answer, network_events):
        ...
        eval_result["num_network_requests"] = len(network_events)
